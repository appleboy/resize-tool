...
function download_and_install() {
  ...
  CLIENT_BINARY="resize-tool-${VERSION}-${CLIENT_PLATFORM}-${CLIENT_ARCH}"
  ...
  # Rename the binary to resize-tool
...
function add_to_path() {
  ...
  if [[ -w $config_file ]]; then
    ...
    print_message info "Successfully added ${ORANGE}resize-tool ${GREEN}to \$PATH in $config_file"
  else
...
function get_latest_version() {
  ...
  if command -v jq >/dev/null 2>&1; then
    latest=$(curl $INSECURE_OPTION -# --retry 5 -fSL https://api.github.com/repos/appleboy/resize-tool/releases/latest | jq -r .tag_name)
  else
    latest=$(curl $INSECURE_OPTION -# --retry 5 -fSL https://api.github.com/repos/appleboy/resize-tool/releases/latest | grep '"tag_name":' | sed -E 's/.*"tag_name": ?"v?([^"]+)".*/\1/')
  fi
...
RELEASE_URL="${RELEASE_URL:-https://github.com/appleboy/resize-tool/releases/download}"
INSTALL_DIR="${INSTALL_DIR:-$HOME/.resize-tool/bin}"
